<p align="center">
  <img alt="Header banner" width="100%" src="https://capsule-render.vercel.app/api?type=venom&color=gradient&height=190&section=header&text=ANURAG%20SONI&fontSize=40&fontColor=FFFFFF&animation=fadeIn&fontAlignY=35&desc=Software%20Engineer%20%7C%20AI%20%2B%20IoT%20%7C%20MERN%20%7C%20DSA&descAlignY=55&descSize=14"/>
</p>

<p align="center">
  <img alt="Typing SVG" width="100%" style="max-width:750px" src="https://readme-typing-svg.demolab.com/?lines=Building+AI+%2B+IoT+systems+that+solve+real+problems;Currently+leveling+up+in+DSA+%26+MERN;Always+shipping+something+new;Open+to+collabs+%26+internships&font=Fira%20Code&center=true&width=750&height=45&color=00D4FF&vCenter=true&size=19"/>
</p>

<p align="center">
  <a href="https://www.linkedin.com/in/anurag-soni-46340533a"><img src="https://img.shields.io/badge/LinkedIn-Profile-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/></a>
  <a href="mailto:anurag1201soni@gmail.com"><img src="https://img.shields.io/badge/Gmail-Contact-D14836?style=for-the-badge&logo=gmail&logoColor=white"/></a>
  <a href="https://leetcode.com/u/Codex_Anurag/"><img src="https://img.shields.io/badge/LeetCode-Profile-FFA116?style=for-the-badge&logo=leetcode&logoColor=white"/></a>
  <a href="https://codeforces.com/profile/anurag_01x"><img src="https://img.shields.io/badge/Codeforces-Profile-1F8ACB?style=for-the-badge&logo=codeforces&logoColor=white"/></a>
  <a href="https://www.codechef.com/users/anurags87"><img src="https://img.shields.io/badge/CodeChef-Profile-5B4638?style=for-the-badge&logo=codechef&logoColor=white"/></a>
</p>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=CodeX-Anurag&label=Profile%20Views&color=00d4ff&style=flat" />
</p>

---

### 🚀 About Me

I'm a Computer Science Engineering student passionate about **Software Development, Data Structures & Algorithms, AI/ML, IoT, and Full-Stack Development.**

- 🔭 Currently working on **AI + IoT projects**
- 🌱 Currently learning **DSA, MERN & AI/ML**
- 💡 Interested in **Software Development & Problem Solving**
- 🎯 Goal: Become a strong **Software Engineer**
- ⚡ I enjoy building projects and solving coding problems

---

### 🛠️ Tech Stack

**Languages**
<p><img src="https://skillicons.dev/icons?i=cpp,python,javascript,c"/></p>

**Web Development**
<p><img src="https://skillicons.dev/icons?i=html,css,javascript,react,nodejs,express,mongodb"/></p>

**AI / Data Science**
<p><img src="https://skillicons.dev/icons?i=python,tensorflow"/></p>

`NumPy` `Pandas` `Matplotlib` `Seaborn` `Scikit-learn` `TensorFlow` `Keras`

**IoT & Embedded Systems**
<p><img src="https://skillicons.dev/icons?i=arduino,raspberrypi"/></p>

`ESP32` `ESP32-CAM` `Arduino` `Raspberry Pi` `Sensors` `Embedded Systems`

**Tools**
<p><img src="https://skillicons.dev/icons?i=git,github,vscode,postman"/></p>

---

### 🔥 Featured Projects

| Project | Description | Tech |
|---|---|---|
| 🔥 **AI Enabled IoT Fire Detection System** | Detects fire hazards early and reduces false alarms using sensor data + visual analysis | `Python` `TensorFlow` `Keras` `ESP32-CAM` `MQ-2` `DHT11` `Flame Sensor` |
| 🚪 **ESP32 Smart Doorbell** | Smart doorbell with camera, notifications, and web-based control | `ESP32-CAM` `MERN` `JavaScript` `Node.js` `MongoDB` |
| 🚗 **Wi-Fi Controlled Car** | ESP32-based car controlled wirelessly over Wi-Fi | `ESP32` `C++` `L298N` `IoT` |
| 🧠 **DSA Practice** | Ongoing journey through core Data Structures & Algorithms in C++ | `Arrays` `Strings` `Sorting` `LinkedList` `Stacks` `Queues` `DP` `Greedy` |

---

### 🧠 DSA Roadmap

```

Arrays → Strings → Searching → Sorting → Linked Lists
   → Stacks & Queues → Trees → Graphs → Greedy → Dynamic Programming

```

---

### 📊 GitHub Stats

<p align="center">
  <img alt="GitHub Stats" height="165" src="https://github-readme-stats.vercel.app/api?username=CodeX-Anurag&show_icons=true&theme=tokyonight&hide_border=true"/>
  <img alt="Top Languages" height="165" src="https://github-readme-stats.vercel.app/api/top-langs/?username=CodeX-Anurag&layout=compact&theme=tokyonight&hide_border=true"/>
</p>

<p align="center">
  <img alt="GitHub Streak Stats" width="100%" style="max-width:700px" src="https://streak-stats.demolab.com/?user=CodeX-Anurag&theme=tokyonight&hide_border=true"/>
</p>

<p align="center">
  <img alt="GitHub Activity Graph" width="100%" src="https://github-readme-activity-graph.vercel.app/graph?username=CodeX-Anurag&theme=react-dark&hide_border=true&area=true"/>
</p>

---

### 🏆 GitHub Trophies

<p align="center">
  <img alt="GitHub Trophies" src="https://github-profile-trophy.vercel.app/?username=CodeX-Anurag&theme=onedark&no-frame=true&margin-w=8&row=1&column=7"/>
</p>

---

### 🐍 Contribution Snake

<p align="center">
  <img alt="Contribution Snake" width="100%" src="https://raw.githubusercontent.com/CodeX-Anurag/CodeX-Anurag/output/github-contribution-grid-snake-dark.svg"/>
</p>

> ⚠️ This image only appears **after** you add the Snake GitHub Action (see setup steps in our earlier conversation, or ask me to resend them).

---

### 🎯 2026 Goals

- [ ] Master DSA
- [ ] Solve 300+ coding problems
- [ ] Build strong MERN projects
- [ ] Build more AI + IoT projects
- [ ] Contribute to Open Source
- [ ] Build production-ready projects
- [ ] Prepare for Software Engineering internships

---

<p align="center"><i>"Code. Learn. Build. Repeat."</i></p>

<img alt="Footer banner" width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=gradient&height=100&section=footer"/>
